...
- Command discovery should include detailed `/bridge` context so bridged providers and local agents understand that bridge swaps the provider runtime while Cast still owns tools, permissions, transcripts, and file/shell guards. It should mention the bare `/bridge` picker, Tab autostart, the picker `Stop bridge` option, and that normal prompts route to the bridge until `/bridge stop`.
