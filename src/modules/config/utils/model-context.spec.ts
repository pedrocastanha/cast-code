...
test('getModelContextWindow returns official windows for configured hosted models', () => {
  ...
  assert.equal(getModelContextWindow('openai', 'gpt-5.4'), 400_000);
  assert.equal(getModelContextWindow('openai', 'gpt-5-mini'), 400_000);
  ...
  assert.equal(getModelContextWindow('qwen', 'qwen-plus-latest'), 1_000_000);
  assert.equal(getModelContextWindow('qwen', 'qwen3-235b-a22b-thinking-2507'), 262_144);
  assert.equal(getModelContextWindow('glm', 'glm-4.5-flash'), 128_000);
  assert.equal(getModelContextWindow('glm', 'glm-4.6'), 200_000);
  assert.equal(getModelContextWindow('mistral', 'mistral-large-latest'), 256_000);
  assert.equal(getModelContextWindow('xai', 'grok-4.3'), 1_000_000);
  assert.equal(getModelContextWindow('cohere', 'command-a-plus-05-2026'), 128_000);
});
...
test('getModelContextWindow maps OpenRouter and local model aliases to their backing windows', () => {
  ...
  assert.equal(getModelContextWindow('openrouter', 'moonshotai/kimi-k2.6'), 256_000);
  assert.equal(getModelContextWindow('openrouter', 'google/gemma-3-27b-it'), 128_000);
