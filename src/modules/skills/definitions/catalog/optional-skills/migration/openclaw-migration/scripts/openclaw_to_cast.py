#!/usr/bin/env python3
"""OpenClaw -> Cast migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Cast Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Cast.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Cast memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Cast memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Cast-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Cast-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Cast command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.cast/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.cast/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Cast .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Cast .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Cast .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Cast .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Cast .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Cast config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Cast config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Cast.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Cast MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Cast config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'cast cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Cast config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Cast session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Cast custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Cast config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Cast config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Cast config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Cast config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Cast in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Cast`` (prose), but lowercase matches
# like ``openclaw`` → ``cast`` (so filesystem paths like ``~/.openclaw``
# become ``~/.cast`` — the real Cast home — not the broken ``~/.Cast``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Cast'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Cast'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Cast'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Cast`` but maps ``openclaw`` → ``cast`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.cast/config.yaml`` (the real Cast home) instead of the broken
    ``~/.Cast/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Cast.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Cast Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-cast/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Cast config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-cast/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Cast session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Cast config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Cast
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Cast-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Cast-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in ("file", "exec"):
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: cast config set",
                        )
                    continue
        ...
        for oc_key, cast_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and cast_key not in secret_additions:
                secret_additions[cast_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, cast_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and cast_key not in secret_additions:
                        secret_additions[cast_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        cast_config = load_yaml_file(destination)
        current_model = cast_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = cast_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                cast_config["model"] = {"default": model_str}
            dump_yaml_file(destination, cast_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in ("elevenlabs", "openai", "edge", "microsoft"):
            # OpenClaw renamed "edge" to "microsoft"; Cast still uses "edge"
        ...
        cast_config = load_yaml_file(destination)
        existing_tts = cast_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Cast destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Cast-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Cast-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        cast_cfg_path = self.target_root / "config.yaml"
        cast_cfg = load_yaml_file(cast_cfg_path)
        existing_mcp = cast_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Cast config")
                continue

            cast_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                cast_srv["command"] = srv["command"]
                if srv.get("args"):
                    cast_srv["args"] = srv["args"]
                if srv.get("env"):
                    cast_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    cast_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                cast_srv["url"] = srv["url"]
                if srv.get("headers"):
                    cast_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    cast_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                cast_srv["enabled"] = False
            if srv.get("timeout"):
                cast_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                cast_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                cast_srv["tools"] = {}
                if tools_cfg.get("include"):
                    cast_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    cast_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(cast_cfg_path)
            cast_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(cast_cfg_path, cast_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'cast cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = cast_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Cast reasoning_effort
        ...
        if compaction:
            compression = cast_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'cast gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("CAST_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        cast_cfg_path = self.target_root / "config.yaml"
        cast_cfg = load_yaml_file(cast_cfg_path)
        sr = cast_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        cast_cfg_path = self.target_root / "config.yaml"
        cast_cfg = load_yaml_file(cast_cfg_path)
        custom_providers = cast_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(cast_cfg_path)
            cast_cfg["custom_providers"] = custom_providers
            dump_yaml_file(cast_cfg_path, cast_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Cast config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            cast_cfg_path = self.target_root / "config.yaml"
            cast_cfg = load_yaml_file(cast_cfg_path)
            discord_cast = cast_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_cast["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_cast["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                cast_cfg["discord"] = discord_cast
                dump_yaml_file(cast_cfg_path, cast_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        cast_cfg_path = self.target_root / "config.yaml"
        cast_cfg = load_yaml_file(cast_cfg_path)
        browser_cast = cast_cfg.get("browser") or {}
        changed = False

        # Map fields that have Cast equivalents
        if browser.get("cdpUrl"):
            browser_cast["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_cast["headless"] = browser["headless"]
            changed = True

        if changed:
            cast_cfg["browser"] = browser_cast
            if self.execute:
                self.maybe_backup(cast_cfg_path)
                dump_yaml_file(cast_cfg_path, cast_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            cast_mode = mode_map.get(mode, "manual")
            cast_cfg.setdefault("approvals", {})["mode"] = cast_mode
            if self.execute:
                self.maybe_backup(cast_cfg_path)
                dump_yaml_file(cast_cfg_path, cast_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{cast_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Cast. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Cast Config Not Overwritten)",
                "",
                "These items already existed in your Cast config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Cast agent discovers these",
            "directories, it may read/write to them instead of the Cast state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `cast claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `cast claw cleanup` to fix it.",
            "",
            "## Cast-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `cast claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `cast setup` to configure any remaining settings",
            "- Run `cast mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `cast cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `cast cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `cast gateway install` if you need the gateway service",
            "- Review `~/.cast/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Cast Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("CAST_HOME") or str(Path.home() / ".cast"), help="Cast home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Cast targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Cast-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Cast Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.cast/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.cast/config.yaml")
        print("    2. Run: cast mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: cast cron")
