...
description: Migrate a user's OpenClaw customization footprint into Cast Agent. Imports Cast-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Cast Agent (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  cast:
    tags: [Migration, OpenClaw, Cast, Memory, Persona, Import]
    related_skills: [cast-agent]
---

# OpenClaw -> Cast Migration

Use this skill when a user wants to move their OpenClaw setup into Cast Agent with minimal manual cleanup.
...
```bash
cast claw migrate              # Full interactive migration
cast claw migrate --dry-run    # Preview what would be migrated
cast claw migrate --preset user-data   # Migrate without secrets
cast claw migrate --overwrite  # Overwrite existing conflicts
cast claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `cast setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_cast.py` to:

- import `SOUL.md` into the Cast home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Cast memory entries
- merge OpenClaw command approval patterns into Cast `command_allowlist`
- migrate Cast-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.cast/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Cast workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.cast/tts/`
- archive non-secret docs that do not have a direct Cast destination
...
- `scripts/openclaw_to_cast.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py`

Do not guess a shorter path like `~/.cast/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.cast/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Cast-compatible secrets, currently:
...
Cast CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Cast. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Cast workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Cast skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.cast/skills/migration/openclaw-migration/scripts/openclaw_to_cast.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Cast unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Cast targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Cast destination. Unsupported auth blobs must still be reported as skipped.
...
- Cast persona state imported
- Cast memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.cast/skills/openclaw-imports/`
