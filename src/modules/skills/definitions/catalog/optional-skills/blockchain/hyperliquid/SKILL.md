...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Cast Agent
license: MIT
...
The script reads `~/.cast/.env` for two optional defaults:
...
Helper script: `~/.cast/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.cast/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.cast/.env`.
...
```bash
python3 ~/.cast/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.cast/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
