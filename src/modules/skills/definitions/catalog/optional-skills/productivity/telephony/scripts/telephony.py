#!/usr/bin/env python3
"""Telephony helper for the Cast optional telephony skill.

Capabilities:
- Persist telephony provider credentials to ~/.cast/.env
...
def _cast_home() -> Path:
    return Path(os.environ.get("CAST_HOME", "~/.cast")).expanduser()


def _env_path() -> Path:
    return _cast_home() / ".env"


def _config_path() -> Path:
    return _cast_home() / "config.yaml"


def _state_path() -> Path:
    return _cast_home() / "telephony_state.json"
...
def _load_root_config() -> dict[str, Any]:
    ...
    if not path.exists():
        return {}
    try:
        import yaml  # optional dependency; Cast already ships PyYAML
    except Exception:
        return {}
    try:
...
def _twilio_creds() -> tuple[str, str]:
    ...
    if not sid or not token:
        raise TelephonyError(
            ...
            "TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN in ~/.cast/.env."
        )
    return sid, token
...
def _resolve_twilio_number(identifier: str | None = None) -> OwnedTwilioNumber:
    ...
    raise TelephonyError(
        ...
        "'twilio-set-default', or set TWILIO_PHONE_NUMBER in ~/.cast/.env."
    )
...
def _vapi_import_twilio_number(
    *,
    ...
    if not api_key:
        raise TelephonyError(
            "Vapi is not configured. Use 'save-vapi' or set VAPI_API_KEY in ~/.cast/.env first."
        )
    owned = _resolve_twilio_number(phone_identifier)
    sid, token = _twilio_creds()
...
def _bland_call(
    ...
    *,
    ...
    if not api_key:
        raise TelephonyError(
            "Bland.ai is not configured. Use 'save-bland' or set BLAND_API_KEY in ~/.cast/.env."
        )
    normalized = _normalize_phone(phone_number)
    if voice is None:
...
def _vapi_call(
    ...
    *,
    ...
    if not api_key:
        raise TelephonyError(
            "Vapi is not configured. Use 'save-vapi' or set VAPI_API_KEY in ~/.cast/.env."
        )
    phone_number_id = _vapi_phone_number_id()
    if not phone_number_id:
        raise TelephonyError(
            "No Vapi phone number id is configured. Import an owned Twilio number with "
            "'vapi-import-twilio --save-env' or set VAPI_PHONE_NUMBER_ID in ~/.cast/.env."
        )
    normalized = _normalize_phone(phone_number)
...
def save_twilio(account_sid: str, auth_token: str, phone_number: str = "", phone_sid: str = "") -> dict[str, Any]:
    ...
    result = {
        ...
        "message": "Twilio credentials saved to ~/.cast/.env.",
    }
    if phone_number:
...
def save_bland(api_key: str, voice: str = BLAND_DEFAULT_VOICE) -> dict[str, Any]:
    ...
    return {
        ...
        "env_path": str(env_file),
        "message": "Bland.ai configuration saved to ~/.cast/.env.",
    }
...
def save_vapi(
    ...
    *,
    ...
    result = {
        ...
        "saved_env_keys": sorted(updates),
        "env_path": str(env_file),
        "message": "Vapi configuration saved to ~/.cast/.env.",
    }
    if phone_number_id:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Cast telephony helper")
    sub = parser.add_subparsers(dest="command", required=True)

    sub.add_parser("diagnose", help="Show saved telephony state and provider readiness")

    p = sub.add_parser("save-twilio", help="Save Twilio credentials to ~/.cast/.env")
    p.add_argument("account_sid")
    p.add_argument("auth_token")
    p.add_argument("--phone-number", default="")
    p.add_argument("--phone-sid", default="")

    p = sub.add_parser("save-bland", help="Save Bland.ai settings to ~/.cast/.env")
    p.add_argument("api_key")
    p.add_argument("--voice", default=BLAND_DEFAULT_VOICE)

    p = sub.add_parser("save-vapi", help="Save Vapi settings to ~/.cast/.env")
    p.add_argument("api_key")
...
def _dispatch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if cmd == "ai-call":
        ...
        raise TelephonyError(
            ...
            "or set PHONE_PROVIDER in ~/.cast/.env."
        )
    if cmd == "ai-status":
        provider = (args.provider or _ai_provider()).lower().strip()
        if provider == "vapi":
