...
- Skill helper paths:
  - `${CAST_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CAST_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CAST_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client cast
```
...
```bash
PUBLISH="${CAST_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client cast
```
...
```bash
DRIVE="${CAST_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `cast`)    |
