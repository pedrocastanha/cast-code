---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. cast-agent@agentmail.to).
version: 1.0.0
...
Add to `~/.cast/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `cast-agent`)
   - The agent gets address: `cast-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "cast-outreach")
...
```
cast --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
