#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `cast kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed cast CLI."""
    try:
        out = subprocess.run(
            ["cast", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `cast kanban list`
    out = subprocess.run(
        ["cast", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not cast_available():
        print("ERROR: 'cast' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
