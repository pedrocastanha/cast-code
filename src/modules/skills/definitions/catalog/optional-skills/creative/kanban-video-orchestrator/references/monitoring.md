...
cast kanban watch --tenant <project-slug>

# Snapshot of the board
cast kanban list --tenant <project-slug>
cast kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
cast kanban stats --tenant <project-slug>

# Visual dashboard (browser)
cast dashboard

# Inspect a specific task (includes comments + events)
cast kanban show <task-id>

# Follow a single task's event stream
cast kanban tail <task-id>
```

Verify available subcommands with `cast kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `cast kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `cast kanban block <id>` then `cast kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `cast kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `cast kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `cast profile list` (profile exists?), `cast status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(cast kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
cast kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
cast kanban block <task-id>
cast kanban archive <task-id>

# Diagnose what happened
cast kanban show <task-id>      # task body, comments, recent events
cast kanban tail <task-id>      # follow the live event stream
cast kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `cast kanban create` for the director
...
while true; do
    clear
    cast kanban list --tenant <slug>
    echo "---"
    cast kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `cast kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `cast profile list` and start
