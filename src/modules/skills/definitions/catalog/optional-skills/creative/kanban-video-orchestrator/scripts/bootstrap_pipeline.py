...
../assets/, and writes a setup.sh that creates Cast profiles and fires the
initial kanban task.

Profile-config patching, SOUL.md-per-profile, TEAM.md task-graph convention,
and the `cast kanban create --workspace dir:` initial-task pattern are
    ...
    if "team" in plan:
        if not isinstance(plan["team"], list) or not plan["team"]:
            ...
            for i, t in enumerate(plan["team"]):
                ...
                # Profile name must match Cast's regex (lowercase
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Cast profile rules"
                        )
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for key in plan.get("api_keys_required", []):
        key_checks.append(f'check_key {key} cast {key} || exit 1')
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'cast profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.cast/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
