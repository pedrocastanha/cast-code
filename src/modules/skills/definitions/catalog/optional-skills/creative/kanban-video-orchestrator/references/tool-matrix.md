...
Maps each role archetype to the Cast skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public cast-agent
...
## Cast skills relevant to video production

### Visual / rendering skills (`cast-agent/skills/creative/`)
...
### Design / pre-production skills (`cast-agent/skills/creative/`)
...
### Audio / media skills (`cast-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`cast-agent/skills/devops/`)
...
These are **not** Cast skills but external CLIs / APIs that profiles invoke.
...
## Built-in Cast tools for media review

These are native Cast tools — not invoked via terminal but through their own
...
key is present in `~/.cast/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Cast profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.cast/.env` → environment variable.
...
When a new Cast-public video skill ships:
