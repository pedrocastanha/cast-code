...
the actual `setup.sh` that creates the project workspace, configures Cast
...
2. **Create profiles** — `cast profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.cast/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `cast kanban create` assigned to the director
...
```bash
cast profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.cast/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.cast/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Cast
...
Each profile gets a `SOUL.md` at `~/.cast/profiles/<name>/SOUL.md` that
...
```bash
cast kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.cast/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.cast/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.cast/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.cast/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY cast ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY cast OPENROUTER_API_KEY || exit 1
# ...
```
