...
Usage (via cron with --no-agent):
    ...
    cast cron create cast-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CAST_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name cast-issues --repo NousResearch/cast-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.cast/.env to avoid the 60 req/hr
