...
Usage (via cron with --no-agent):
    ...
    cast cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CAST_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Cast-Watcher/1.0"})
