---
name: honcho
description: Configure and use Honcho memory with Cast -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Cast Agent
license: MIT
...
# Honcho Memory for Cast

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Cast profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/cast#running-honcho-locally-with-cast

### Verify

```bash
cast honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Cast profile's AI peer
...
Honcho models conversations as interactions between **peers**. Cast creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Cast instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Cast run |
| `global` | Single session across all directories |

Manual override: `cast honcho map my-project-name`
...
Each Cast profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
cast profile create coder --clone
# creates host block cast.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `cast.coder` host block in `honcho.json`
...
```bash
cast honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Cast when Honcho memory is active.
...
Config file: `$CAST_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `cast honcho setup`. Ensure `memory.provider: honcho` is in `~/.cast/config.yaml`.

### Memory not persisting across sessions
Check `cast honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `cast profile create <name> --clone`. For existing profiles: `cast honcho sync`.
...
| Command | Description |
|---------|-------------|
| `cast honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `cast honcho status` | Show resolved config, connection test, peer info for active profile |
| `cast honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `cast honcho disable` | Disable Honcho for the active profile |
| `cast honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `cast honcho peers` | Show peer identities across all profiles |
| `cast honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `cast honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `cast honcho sessions` | List known directory-to-session-name mappings |
| `cast honcho map <name>` | Map current working directory to a Honcho session name |
| `cast honcho identity` | Seed AI peer identity or show both peer representations |
| `cast honcho sync` | Create host blocks for all Cast profiles that don't have one yet |
| `cast honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Cast + Honcho |
| `cast memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `cast memory status` | Show active memory provider and config |
| `cast memory off` | Disable external memory provider |
