...
version: 1.0.0
author: arceus77-7, enhanced by Cast Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Cast terminal calls (desktop app flow only)
...
### Service Account (recommended for Cast)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.cast/.env` (the skill will prompt for this on first load).
...
## Cast Execution Pattern (desktop app flow)

Cast terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/cast-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/cast-op.sock"
