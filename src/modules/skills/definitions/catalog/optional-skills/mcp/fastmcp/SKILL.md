...
- smoke-test a server with the FastMCP CLI before wiring it into Cast or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Cast. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cast/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cast/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Cast integration, either:

- configure the server in `~/.cast/config.yaml` using the `native-mcp` skill, or
...
### Cast cannot see the deployed server

The server-building part may be correct while the Cast config is not. Load the `native-mcp` skill and configure the server in `~/.cast/config.yaml`, then restart Cast.
