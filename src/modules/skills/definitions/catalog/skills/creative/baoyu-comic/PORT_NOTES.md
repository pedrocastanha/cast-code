...
| Change | Upstream | Cast |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `cast` (with `tags` + `homepage`) |
| Trigger | Slash commands / CLI flags | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Cast infra |
...
| File operations | Generic instructions | Cast file tools (`write_file`, `read_file`) |
...
- **`scripts/` directory** (removed entirely): upstream's `merge-to-pdf.ts` depended on `pdf-lib`, which is not declared anywhere in the Cast repo. Rather than add a new dependency, the port drops PDF assembly and delivers per-page PNGs.
