...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Cast uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Cast has `image_generate`
...
- License: MIT (inherits from cast-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Cast Agent contributors
