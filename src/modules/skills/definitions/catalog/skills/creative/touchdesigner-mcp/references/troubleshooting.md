...
## 5. Cast Configuration

### Config location

`$CAST_HOME/config.yaml` (defaults to `~/.cast/config.yaml` when `CAST_HOME` is unset)
...
Restart the Cast session for changes to take effect. The MCP connection is
