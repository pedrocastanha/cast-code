...
metadata:
  cast:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, cast-video]
...
```
Cast Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${CAST_HOME:-$HOME/.cast}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Cast config (if missing)
...
3. **Restart Cast session** to pick up the new MCP server
