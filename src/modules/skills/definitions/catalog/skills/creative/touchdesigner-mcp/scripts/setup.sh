...
CAST_HOME_DIR="${CAST_HOME:-$HOME/.cast}"
CAST_CFG="${CAST_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Cast config has twozero_td MCP entry ──
if [[ ! -f "$CAST_CFG" ]]; then
    echo -e " ${FAIL} Cast config not found at ${CAST_CFG}"
    manual_steps+=("Create ${CAST_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CAST_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Cast config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Cast config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CAST_CFG} manually"); }
    manual_steps+=("Restart Cast session to pick up config change")
fi
