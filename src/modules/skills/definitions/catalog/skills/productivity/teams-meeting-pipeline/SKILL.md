...
description: "Operate the Teams meeting summary pipeline via Cast CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `cast teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.cast/.env`:
...
```bash
cast teams-pipeline validate              # config snapshot — run first after any change
cast teams-pipeline token-health          # Graph token status
cast teams-pipeline token-health --force-refresh   # force a fresh token acquisition
cast teams-pipeline list                  # recent meeting jobs
cast teams-pipeline list --status failed  # only failed jobs
cast teams-pipeline show <job-id>         # full detail of one job
cast teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
cast teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
cast teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
cast teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
cast teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

cast teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
cast teams-pipeline delete-subscription <sub-id>
cast teams-pipeline maintain-subscriptions            # renew near-expiry ones
cast teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `cast teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `cast cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
