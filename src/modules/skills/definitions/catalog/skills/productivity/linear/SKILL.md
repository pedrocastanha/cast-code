...
2. Set `LINEAR_API_KEY` in your environment (via `cast setup` or your env config)
...
```bash
SCRIPT=$(dirname "$(find ~/.cast -path '*skills/productivity/linear/scripts/linear_api.py' 2>/dev/null | head -1)")/linear_api.py
...
The trailing hex segment is the `slugId`. Example: `https://linear.app/nousresearch/document/rfc-cast-permission-gateway-discord-38359beef67c` → `slugId` is `38359beef67c`.
