#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Cast Agent.
...
# Ensure sibling modules (_cast_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _cast_home import display_cast_home, get_cast_home

CAST_HOME = get_cast_home()
TOKEN_PATH = CAST_HOME / "google_token.json"
CLIENT_SECRET_PATH = CAST_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = CAST_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Cast profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'cast-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Cast home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_cast_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Cast")
