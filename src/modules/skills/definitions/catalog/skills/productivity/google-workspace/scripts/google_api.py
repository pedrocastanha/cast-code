#!/usr/bin/env python3
"""Google Workspace API CLI for Cast Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Cast-facing JSON contract and falls back to the Python client
...
from _cast_home import get_cast_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CAST_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Cast Agent")
