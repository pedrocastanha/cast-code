...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CAST_HOME", os.path.expanduser("~/.cast")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CAST_HOME", Path.home() / ".cast")) / "skills" / "red-teaming" / "godmode"
...
CAST_HOME = Path(os.getenv("CAST_HOME", Path.home() / ".cast"))
CONFIG_PATH = CAST_HOME / "config.yaml"
PREFILL_PATH = CAST_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Cast models — already uncensored, just needs clean prompt
    "cast": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "cast" in model_lower or "nous" in model_lower:
        return "cast"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Cast config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cast/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Cast for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Cast for changes to take effect.")
