...
## 5. GODMODE FAST — Cast 4 405B (Zero Refusal)

**Model:** `nousresearch/cast-4-405b`
**Strategy:** Cast models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Cast Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.cast/config.yaml`:
