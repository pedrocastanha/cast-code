...
metadata:
  cast:
    ...
    related_skills: [systematic-debugging, python-debugpy, debugging-cast-tui-commands]
---
...
Cast-specific note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In Cast, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
