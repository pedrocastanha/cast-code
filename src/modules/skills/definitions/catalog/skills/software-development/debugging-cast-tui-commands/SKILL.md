---
name: debugging-cast-tui-commands
description: "Debug Cast TUI slash commands: Python, gateway, Ink UI."
version: 1.0.0
author: Cast Agent
license: MIT
...
metadata:
  cast:
    tags: [debugging, cast-agent, tui, slash-commands, typescript, python]
    related_skills: [python-debugpy, node-inspect-debugger, systematic-debugging]
---

# Debugging Cast TUI Slash Commands
...
Use this skill when you encounter issues with slash commands in the Cast TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
```
Python backend (cast_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path cast_cli/
   search_files --pattern "commandname" --path cast_cli/commands.py --context 3
   ```
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `cast_cli/commands.py`:
   ```python
...
4. If the command runs server-side, add a handler in `CastCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `cast_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/cast-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.cast/config.yaml`)
