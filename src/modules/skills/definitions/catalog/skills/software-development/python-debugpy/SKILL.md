...
metadata:
  cast:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, debugging-cast-tui-commands]
---
...
- A long-running process (cast gateway, tui_gateway) misbehaves and you can't restart it
...
The cast test runner and pytest both support this:
...
For long-lived processes: Cast gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/cast-agent/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Cast you have two practical options:
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/cast-agent" }
  ]
}
```
...
## Debugging Cast-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `cast` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `cast --tui`)
...
debugpy.wait_for_client()
```
Start `cast --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Cast subagents, debug one process at a time.
