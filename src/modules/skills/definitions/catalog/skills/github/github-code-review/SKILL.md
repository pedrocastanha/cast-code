...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.cast/.env ] && grep -q "^GITHUB_TOKEN=" ~/.cast/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.cast/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Cast Agent\",
    \"comments\": [
...
```bash
source "${CAST_HOME:-$HOME/.cast}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Cast Agent. Code looks clean — good test coverage, no security concerns."
