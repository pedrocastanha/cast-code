...
The atomic unit is a hunk (one contiguous block of the diff), identified by "path#index".
...
- EVERY input hunk appears in EXACTLY ONE group. No omissions, no duplicates.
...
[{"name": "...", "responsibility": "one sentence", "commit": "...", "hunks": ["path#0", "path#1", ...]}]`;
...
export function buildBranchSplitPrompt(fileDiffs: FileDiff[]): string {
  ...
  for (const fd of fileDiffs) {
    ...
    fd.hunks.forEach((h, i) => {
      ...
      lines.push(`- ${fd.file}#${i} (+${h.added} −${h.deleted}) ${head}`);
    });
  }
  return `Changed hunks (id, weight, header):\n${lines.join('\n')}\n\nGroup these hunks into an ordered, dependency-first stack.`;
}
